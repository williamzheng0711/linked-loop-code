#!/usr/bin/env python3
"""Fast no-SIC empirical probe for LLC/UACE.

This is a research-side decoder wrapper.  It preserves the current decoder's
"take the first final-valid path per root" rule, but searches for that first
path depth-first instead of materializing the full breadth-first path forest.

The wrapper is intended for phase-III validation runs where the original
`phase2plus_decoder` spends most of its time building false path forests that
are later discarded.
"""

from __future__ import annotations

import argparse
import random
import sys
import time
from dataclasses import dataclass
from pathlib import Path

import numpy as np

from uace_bound_explorer import rank_peeling_succeeds
from uace_empirical_probe import append_unique, rows_as_set, uace_channel
from uace_schedule_bound import schedule_succeeds


@dataclass(frozen=True)
class FastTrialMetrics:
    seed: int
    pdp: float
    php: float
    decoded: int
    correct: int
    false_positive: int
    schedule_fail_emp: float
    rank_fail_emp: float
    seconds: float
    node_visits: int
    aborted_roots: int


def load_playground():
    repo_root = Path(__file__).resolve().parents[1]
    playground = repo_root / "playground"
    sys.path.insert(0, str(playground))
    import abch_utils  # type: ignore
    import general_lib  # type: ignore
    import general_utils as gu  # type: ignore
    import linkedloop as LLC  # type: ignore
    import static_repo  # type: ignore

    return static_repo, general_lib, gu, LLC, abch_utils


def rotate_decoder_state(static_repo, L, M, chosen_roots, grand_list, message_lens, parity_lens, gis, columns_index, sub_g_invs):
    chosen_root = 0 if chosen_roots is None else chosen_roots[-1]
    erasure_slot = [np.mod(0 - root, L) for root in chosen_roots] if chosen_roots is not None else []

    message_lens = message_lens.copy()
    parity_lens = parity_lens.copy()
    gis = gis.copy()
    columns_index = columns_index.copy()
    sub_g_invs = sub_g_invs.copy()
    grand = grand_list.copy()

    message_lens[range(L)] = message_lens[np.mod(np.arange(chosen_root, chosen_root + L), L)]
    parity_lens[range(L)] = parity_lens[np.mod(np.arange(chosen_root, chosen_root + L), L)]
    gis[range(L)] = gis[np.mod(np.arange(chosen_root, chosen_root + L), L)]
    columns_index[range(L)] = columns_index[np.mod(np.arange(chosen_root, chosen_root + L), L)]
    sub_g_invs[range(L)] = sub_g_invs[np.mod(np.arange(chosen_root, chosen_root + L), L)]
    grand[:, range(L * static_repo.J)] = grand[:, np.mod(np.arange(chosen_root * static_repo.J, chosen_root * static_repo.J + L * static_repo.J), L * static_repo.J)]
    gijs = static_repo.partition_Gs(L, M, parity_lens, gis)
    return chosen_root, erasure_slot, grand, message_lens, parity_lens, gis, columns_index, sub_g_invs, gijs


def unrotate_messages(chosen_root: int, decoded_msg: np.ndarray, message_lens: np.ndarray) -> np.ndarray:
    if decoded_msg.size == 0:
        return decoded_msg
    w = int(sum(message_lens))
    decoded_msg = decoded_msg.copy()
    decoded_msg[:, range(w)] = decoded_msg[:, np.mod(np.arange(w) + sum(message_lens[0:L_GLOBAL - chosen_root]), w)]
    return decoded_msg


L_GLOBAL = 16


def first_valid_path_for_root(
    *,
    root: int,
    d: int,
    grand,
    K: int,
    L: int,
    M: int,
    message_lens,
    parity_lens,
    gis,
    gijs,
    columns_index,
    sub_g_invs,
    erasure_slot,
    caches,
    LLC,
    max_nodes: int,
) -> tuple[object | None, int, bool]:
    valid_ks_by_section, parity_cache, deciders_cache, avail_savers_cache, parity_lookup_by_section, solve_cache = caches

    start = LLC.GLinkedLoop([root], message_lens)
    stack: list[tuple[int, object]] = [(1, start)]
    visits = 0

    while stack:
        section, path = stack.pop()
        visits += 1
        if visits > max_nodes:
            return None, visits, True

        if section == L:
            if gu_global.final_parity_check_oop(
                path,
                grand,
                message_lens,
                parity_lens,
                L,
                gijs,
                M,
                parity_cache=parity_cache,
                deciders_cache=deciders_cache,
            ):
                return path, visits, False
            continue

        children = gu_global.Path_goes_section_l(
            section,
            path,
            d,
            grand,
            K,
            message_lens,
            parity_lens,
            L,
            M,
            gis,
            gijs,
            columns_index,
            sub_g_invs,
            erasure_slot,
            valid_ks_by_section=valid_ks_by_section,
            parity_cache=parity_cache,
            deciders_cache=deciders_cache,
            avail_savers_cache=avail_savers_cache,
            parity_lookup_by_section=parity_lookup_by_section,
            solve_cache=solve_cache,
        )
        # Reverse push preserves the breadth-first child order when searching
        # depth-first for the first final-valid path.
        for child in reversed(children):
            stack.append((section + 1, child))

    return None, visits, False


gu_global = None


def fast_phase2plus_decoder(
    *,
    static_repo,
    gu,
    LLC,
    d: int,
    grand_list: np.ndarray,
    L: int,
    M: int,
    K: int,
    message_lens: np.ndarray,
    parity_lens: np.ndarray,
    gis: np.ndarray,
    columns_index: np.ndarray,
    sub_g_invs: np.ndarray,
    chosen_roots: list[int] | None,
    max_nodes_per_root: int,
) -> tuple[np.ndarray, int, int]:
    global gu_global, L_GLOBAL
    gu_global = gu
    L_GLOBAL = L

    chosen_root, erasure_slot, grand, msg_lens, par_lens, rot_gis, rot_columns, rot_invs, gijs = rotate_decoder_state(
        static_repo, L, M, chosen_roots, grand_list, message_lens, parity_lens, gis, columns_index, sub_g_invs
    )
    deciders_cache, avail_savers_cache = gu.build_decoder_lookup(L, M)
    solve_cache = gu.build_solve_cache(L, M, rot_columns, rot_invs, rot_gis)
    valid_ks_by_section = [np.flatnonzero(grand[:, l * static_repo.J] != -1) for l in range(L)]
    parity_lookup_by_section = gu.build_parity_lookup_by_section(grand, L, msg_lens, valid_ks_by_section)
    parity_cache = gu.build_parity_cache(grand, L, M, msg_lens, gijs)
    caches = (valid_ks_by_section, parity_cache, deciders_cache, avail_savers_cache, parity_lookup_by_section, solve_cache)

    decoded_chunks = []
    node_visits = 0
    aborted_roots = 0
    k_effective = [x for x in range(K) if grand[x, 0] != -1]
    for root in k_effective:
        path, visits, aborted = first_valid_path_for_root(
            root=root,
            d=d,
            grand=grand,
            K=K,
            L=L,
            M=M,
            message_lens=msg_lens,
            parity_lens=par_lens,
            gis=rot_gis,
            gijs=gijs,
            columns_index=rot_columns,
            sub_g_invs=rot_invs,
            erasure_slot=erasure_slot,
            caches=caches,
            LLC=LLC,
            max_nodes=max_nodes_per_root,
        )
        node_visits += visits
        aborted_roots += int(aborted)
        if path is not None:
            recovered = gu.output_message_oop(grand, [path], L, static_repo.J)
            decoded_chunks.append(recovered)

    decoded_msg = np.vstack(decoded_chunks) if decoded_chunks else np.empty(shape=(0, 0), dtype=int)
    if decoded_msg.size:
        w = int(sum(msg_lens))
        decoded_msg[:, range(w)] = decoded_msg[:, np.mod(np.arange(w) + sum(msg_lens[0:L - chosen_root]), w)]
        decoded_msg = np.unique(decoded_msg, axis=0)
    return decoded_msg, node_visits, aborted_roots


def run_fast_trial(args: argparse.Namespace, seed: int) -> FastTrialMetrics:
    static_repo, general_lib, gu, LLC, abch_utils = load_playground()
    rng = np.random.default_rng(seed)
    random.seed(seed)
    np.random.seed(seed)
    start = time.time()

    message_lens, parity_lens = static_repo.get_allocation(args.L)
    gis, columns_index, sub_g_invs = static_repo.get_G_info(
        args.L, args.M, message_lens, parity_lens, seed=seed
    )
    gijs = static_repo.partition_Gs(args.L, args.M, parity_lens, gis)

    tx_bits = rng.integers(0, 2, size=(args.K, static_repo.B), dtype=int)
    tx_cdwds = general_lib.encode(
        tx_bits,
        args.K,
        args.L,
        args.L * static_repo.J,
        args.M,
        message_lens,
        parity_lens,
        gijs,
    )
    tx_symbols = abch_utils.binary_to_symbol(tx_cdwds, args.L, args.K)
    rx_symbols, _erasure_counts, erasure_masks = uace_channel(tx_symbols, args.pe, rng)
    grand_list = abch_utils.symbol_to_binary(args.K, args.L, rx_symbols)

    schedule_fail_emp = float(
        np.mean(
            [
                not schedule_succeeds(
                    int(mask),
                    length=args.L,
                    memory=args.M,
                    phase=args.phase,
                    message_lens=message_lens,
                    gijs=gijs,
                )
                for mask in erasure_masks
            ]
        )
    )
    rank_fail_emp = float(
        np.mean(
            [
                not rank_peeling_succeeds(
                    int(mask),
                    args.L,
                    args.M,
                    message_lens,
                    gijs,
                )
                for mask in erasure_masks
            ]
        )
    )

    decoded = np.empty(shape=(0, 0), dtype=int)
    node_visits = 0
    aborted_roots = 0

    rx_p1, _ = general_lib.phase1_decoder(
        grand_list.copy(),
        args.L,
        gijs,
        message_lens,
        parity_lens,
        args.K,
        args.M,
        SIC=False,
        toPrint=False,
    )
    decoded = append_unique(decoded, rx_p1)

    if args.phase >= 2:
        for d, roots in [(1, None), (1, [8])]:
            rx, visits, aborted = fast_phase2plus_decoder(
                static_repo=static_repo,
                gu=gu,
                LLC=LLC,
                d=d,
                grand_list=grand_list,
                L=args.L,
                M=args.M,
                K=args.K,
                message_lens=message_lens,
                parity_lens=parity_lens,
                gis=gis,
                columns_index=columns_index,
                sub_g_invs=sub_g_invs,
                chosen_roots=roots,
                max_nodes_per_root=args.max_nodes_per_root,
            )
            decoded = append_unique(decoded, rx)
            node_visits += visits
            aborted_roots += aborted

    if args.phase >= 3:
        for roots in (None, [6], [6, 10]):
            rx, visits, aborted = fast_phase2plus_decoder(
                static_repo=static_repo,
                gu=gu,
                LLC=LLC,
                d=2,
                grand_list=grand_list,
                L=args.L,
                M=args.M,
                K=args.K,
                message_lens=message_lens,
                parity_lens=parity_lens,
                gis=gis,
                columns_index=columns_index,
                sub_g_invs=sub_g_invs,
                chosen_roots=roots,
                max_nodes_per_root=args.max_nodes_per_root,
            )
            decoded = append_unique(decoded, rx)
            node_visits += visits
            aborted_roots += aborted

    true_set = rows_as_set(tx_bits)
    decoded_set = rows_as_set(decoded)
    correct = len(true_set & decoded_set)
    false_positive = len(decoded_set - true_set)
    return FastTrialMetrics(
        seed=seed,
        pdp=1.0 - correct / args.K,
        php=false_positive / args.K,
        decoded=len(decoded_set),
        correct=correct,
        false_positive=false_positive,
        schedule_fail_emp=schedule_fail_emp,
        rank_fail_emp=rank_fail_emp,
        seconds=time.time() - start,
        node_visits=node_visits,
        aborted_roots=aborted_roots,
    )


def summarize(metrics: list[FastTrialMetrics], args: argparse.Namespace) -> str:
    def mean(field: str) -> float:
        return float(np.mean([getattr(item, field) for item in metrics]))

    lines = [
        "# Fast UACE Empirical Probe",
        "",
        f"- `K = {args.K}`",
        f"- `L = {args.L}`",
        f"- `M = {args.M}`",
        f"- `pe = {args.pe}`",
        f"- `phase = {args.phase}`",
        f"- trials: `{args.trials}`",
        f"- max nodes per root: `{args.max_nodes_per_root}`",
        "",
        "| metric | mean |",
        "|---|---:|",
    ]
    for field in (
        "pdp",
        "php",
        "decoded",
        "correct",
        "false_positive",
        "schedule_fail_emp",
        "rank_fail_emp",
        "seconds",
        "node_visits",
        "aborted_roots",
    ):
        lines.append(f"| {field} | {mean(field):.6f} |")
    lines.extend(
        [
            "",
            "Per-trial rows:",
            "",
            "| seed | PDP | PHP | decoded | correct | false positive | schedule fail | rank fail | seconds | node visits | aborted roots |",
            "|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        ]
    )
    for item in metrics:
        lines.append(
            f"| {item.seed} | {item.pdp:.6f} | {item.php:.6f} | {item.decoded} | "
            f"{item.correct} | {item.false_positive} | {item.schedule_fail_emp:.6f} | "
            f"{item.rank_fail_emp:.6f} | {item.seconds:.2f} | {item.node_visits} | {item.aborted_roots} |"
        )
    lines.extend(
        [
            "",
            "Note: this fast wrapper is exact only when `aborted_roots = 0`.  If roots abort, the PDP is an upper-biased drop estimate because those roots are treated as undecoded.",
            "",
        ]
    )
    return "\n".join(lines)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--K", type=int, default=30)
    parser.add_argument("--L", type=int, default=16)
    parser.add_argument("--M", type=int, default=3)
    parser.add_argument("--pe", type=float, default=0.3)
    parser.add_argument("--phase", type=int, default=3, choices=(2, 3))
    parser.add_argument("--trials", type=int, default=1)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--max-nodes-per-root", type=int, default=1_000_000)
    parser.add_argument("--output", type=Path, default=None)
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    metrics = [run_fast_trial(args, args.seed + idx) for idx in range(args.trials)]
    content = summarize(metrics, args)
    if args.output:
        args.output.write_text(content + "\n", encoding="utf-8")
        print(f"wrote {args.output}")
    else:
        print(content)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
