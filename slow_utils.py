...
def slow_parity_check(Parity_computed,Path,k,cs_decoded_tx_message,J,messageLengthVector, parityDistribution, useWhichMatrix, L, windowSize):
    Lpath = Path.shape[1] # 當Lpath<16 也是 現在target的section 的意思
    focusPath = Path[0]
    losts = np.where( focusPath < 0 )[0]

    m = messageLengthVector[0]
    generator_matrices = matrix_repo(dim=m)
    inv_generator_matrices = matrix_inv_repo(dim=m)
    ...
    if Lpath < L:  # Path 還在生長階段
        if len(losts) == 0: # 沒有 lost 最簡單的情況
            ...
            availSavers = [ saver for saver in saverSections if (saver <= Lpath and np.mod(saver-1,L) <= Lpath and np.mod(saver-2,L) <= Lpath and np.mod(saver-3,L) <= Lpath and np.mod(saver-4,L) <= Lpath) ]
...
def slow_recover_msg(sectionLost, decoded_message, parityDistribution, messageLengthVector, J, L, useWhichMatrix):
    generator_matrices = matrix_repo(dim=8)
    inv_generator_matrices = matrix_inv_repo(dim=8)
...
def slow_decode_deal_with_root_i(i,L,cs_decoded_tx_message, J,parityInvolved, whichGMatrix, messageLengthVector, listSize,parityLengthVector, sigValues, windowSize):
    # Every i is a root.
    Paths = np.array([[i]])
    p = parityLengthVector[0]
    for l in range(1, L):
        ...
        for j in range(Paths.shape[0]):
            ...
            Parity_computed= np.ones((1,p),dtype=int)
            if l >= windowSize:
                Parity_computed = slow_compute_permissible_parity(Path, cs_decoded_tx_message, J, parityInvolved, l, whichGMatrix)
            for k in range(listSize):
                index = l<windowSize or slow_parity_check(Parity_computed, Path, k, cs_decoded_tx_message, J, messageLengthVector, parityInvolved, whichGMatrix, L, windowSize)
...
def slow_correct_each_section_and_path(l, j, Paths, cs_decoded_tx_message, J, parityInvolved, whichGMatrix, listSize, messageLengthVector, L, windowSize):
    ...
    for k in range(listSize):
        if l < 4:
            ...
            index = slow_parity_check(Parity_computed, Path, k, cs_decoded_tx_message, J, messageLengthVector, parityInvolved, whichGMatrix, L, windowSize) 
            if index:
                new = np.vstack((new,np.hstack((Path.reshape(1,-1),np.array([[k]]))))) if new.size else np.hstack((Path.reshape(1,-1),np.array([[k]])))
    if len(pathArgNa) == 0:
