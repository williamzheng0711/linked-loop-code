...
def slow_parity_check(Parity_computed,Path,k,cs_decoded_tx_message,J,messageLen, parityDistribution, useWhichMatrix, L, windowSize):
...
def slow_recover_msg(sectionLost, decoded_message, parityDistribution, messageLen, J, L, useWhichMatrix):
    ...
    for l in range(1, L):
        ...
        for j in range(Paths.shape[0]):
            ...
            for k in range(listSize):
                index = l<windowSize or slow_parity_check(Parity_computed, Path, k, cs_decoded_tx_message, J, messageLen, parityInvolved, whichGMatrix, L, windowSize)
...
def slow_correct_each_section_and_path(l, j, Paths, cs_decoded_tx_message, J, parityInvolved, whichGMatrix, listSize, messageLen, parityLen, L, windowSize):
    ...
    for k in range(listSize):
        if l < 4:
            ...
            index = slow_parity_check(Parity_computed, Path, k, cs_decoded_tx_message, J, messageLen, parityInvolved, whichGMatrix, L, windowSize) 
            if index:
                new = np.vstack((new,np.hstack((Path.reshape(1,-1),np.array([[k]]))))) if new.size else np.hstack((Path.reshape(1,-1),np.array([[k]])))
    if len(pathArgNa) == 0:
